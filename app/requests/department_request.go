...
func DepartmentUpdate(data interface{}, c *gin.Context) map[string][]string {

	rules := govalidator.MapData{
		"parent_id": []string{"required", "exists_or_zero:departments,id"},
		"name":      []string{"required", "min_cn:2", "max_cn:60"},
	}
	messages := govalidator.MapData{
		"parent_id": []string{
			"required: 部门信息为必输项",
			"exists_or_zero: 查询不到部门信息",
		},
		"name": []string{
			"required:名称为必填项",
			"min_cn:名称长度需至少 2 个字",
